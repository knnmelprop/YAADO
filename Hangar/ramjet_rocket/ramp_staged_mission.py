...
:class:`IADE_Core.mission_builder.MissionBuilder`, defining the sequence of
...
from IADE_Core.Foundation.mission_builder import MissionBuilder, MissionSegment  # noqa: E402
...
def _vehicle_stage1_boost_params() -> tuple[float, float]:
    ...
    config_path = (
        this_dir.parent.parent / "Hangar" / "ramjet_rocket" / "vehicle_config.yaml"
    )
...
def _estimate_cruise_drag_N(
    ...
    """
    from IADE_Core.modules.powerplant.inlet_methods.wedge import isa_atmosphere
...
def _compute_ramjet_design_point() -> dict[str, Any]:
    ...
    Returns:
        ...
        ``**kwargs`` to :meth:`IADE_Core.mission_builder.MissionBuilder.add_segment`.
    """
    fuel_mass_kg = 15.0  # SZACOWANY — placeholder stage-2 fuel load
    ...
    try:
        ...
        return {
            ...
            "note": (
                ...
                "IADE_Core.modules.powerplant.cycle_methods.grzywka."
...
def main() -> None:
    ...
    """
    this_dir = Path(__file__).resolve().parent
    burnout_json_path = this_dir.parent / "IADE_Core" / "modules" / "trajectory" / "burnout_state.json"
