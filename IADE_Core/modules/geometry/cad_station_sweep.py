...
"""Production STEP cross-section station sweep for the ramP rocket CAD.
...
def cross_check_against_vehicle_config(
    ...
    config = BaseVehicleConfig.from_yaml(vehicle_config_path)
    if not isinstance(config, RocketConfig):
        raise TypeError(
            f"{vehicle_config_path} did not load as a RocketConfig "
            f"(got {type(config).__name__}); this cross-check is for "
            "Project B (ramjet_rocket) only"
        )
