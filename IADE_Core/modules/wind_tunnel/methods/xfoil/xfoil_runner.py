...
1. Checks :mod:`IADE_Core.solver_registry` for a registered ``xfoil`` binary.
...
class XFOILAnalysis(BaseAnalysis):
        ...
        for mach in case.mach_list:
            if mach <= MIN_MACH_ABSOLUTE:
                raise ValueError(
                    ...
                    "IADE_Core.modules.stability_control.methods.barrowman.barrowman_stability) instead"
                )
    ...
    def execute(self) -> AnalysisResults:
        ...
        Checks :mod:`IADE_Core.solver_registry` for a registered/available
