...
from IADE_Core.modules.propulsion.combustor_nozzle_cycle import GrzywkaCombustorNozzleAnalysis
from IADE_Core.modules.propulsion.inlet_performance import mil_e_5007_eta_std
from IADE_Core.Inspectors.vehicle_schema import BaseVehicleConfig, RocketConfig
...
VEHICLE_CONFIG_PATH: Path = (
    Path(__file__).resolve().parents[3] / "Hangar" / "ramjet_rocket" / "vehicle_config.yaml"
)
...
BURNOUT_STATE_JSON_PATH: Path = (
    Path(__file__).resolve().parents[3] / "IADE_Core" / "modules" / "trajectory" / "burnout_state.json"
)
...
def reference_area_m2(config_path: Path = VEHICLE_CONFIG_PATH) -> float:
    ...
    Args:
        config_path: Path to ``Hangar/ramjet_rocket/vehicle_config.yaml``.
